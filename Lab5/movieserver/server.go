...
func (s *server) SetMovieInfo(ctx context.Context, in *movieapi.MovieData) (*movieapi.Status, error) {
	...
	castSlice := in.GetCast()
	cast := strings.Join(castSlice, " ")
	log.Printf("Received: Set %v, Year: %v, Director: %v, Cast: %v", title, year, director, cast)
	reply := &movieapi.Status{}
	if _, ok := moviedb[title]; ok {
		reply.Status = "The movie is already existed"
	} else {
		var val []string
		val = append(val, year, director, cast) //append year, director, cast
		moviedb[title] = val                    //put the new movie in the maps
		reply.Status = "\"" + title + "\"" + " is add to database"
	}
	return reply, nil
}
